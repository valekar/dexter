...
export async function initializeProviders(): Promise<void> {
  try {
    ...
    logger.info(`Initialized ${available.length} providers:`, available.map(p => p.config.id).join(', '));
  } catch (error) {
    logger.warn('Failed to initialize provider registry:', error instanceof Error ? error.message : String(error));
  }
}
